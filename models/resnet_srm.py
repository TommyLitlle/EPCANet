...
class SELayer(nn.Module):
    ...
    def forward(self, x):
        ...
        y = self.fc(y)
        y= sparsemax(y,dim=1).view(b, c, 1, 1)
        return x * y
...
class DSELayer(nn.Module):
    ...
    def forward(self, x):  
        ...
        channel_std = channel_var.sqrt().view(b,c,1,1)
        t = torch.cat((GAP, DAP,channel_std), dim=2)
        
        y = self.conv(t).view(b, c)
        #print(y.size())
        out =  self.activation(y).view(b, c, 1, 1)
        #out = self.activation(y)*x
...
class SRMLayer(nn.Module):
    ...
    def _style_pooling(self, x, eps=1e-5):
        ...
        channel_max, idx = x.view(N, C, -1).max(dim=2, keepdim=True)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.se  = SRMLayer(planes * self.expansion)
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
...
def spe_resnet152(pretrained=False, **kwargs):
